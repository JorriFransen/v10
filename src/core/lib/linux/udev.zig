...
fn ref_stub(udev: *Udev) callconv(.c) ?*Udev {
    _ = udev;
    return null;
}
...
fn unref_stub(udev: *Udev) callconv(.c) ?*Udev {
    _ = udev;
    return null;
}
...
fn device_ref_stub(device: *Device) callconv(.c) ?*Device {
    _ = device;
    return null;
}
const FN_device_ref = @TypeOf(device_ref_stub);
pub var device_ref: *const FN_device_ref = undefined;
...
fn device_new_from_syspath_stub(udev: *Udev, syspath: ?[*:0]const u8) callconv(.c) ?*Device {
    _ = udev;
    _ = syspath;
    return null;
}
...
fn device_get_sysattr_value_stub(device: *Device, sysattr: [*:0]const u8) callconv(.c) ?[*:0]const u8 {
    _ = device;
    _ = sysattr;
    return null;
}
const FN_device_get_sysattr_value = @TypeOf(device_get_sysattr_value_stub);
pub var device_get_sysattr_value: *const FN_device_get_sysattr_value = undefined;
...
fn monitor_new_from_netlink_stub(udev: *Udev, name: ?[*:0]const u8) callconv(.c) ?*Monitor {
    _ = udev;
    _ = name;
    return null;
}
...
fn enumerate_ref_stub(enumerate: *Enumerator) callconv(.c) ?*Enumerator {
    _ = enumerate;
    return null;
}
const FN_enumerate_ref = @TypeOf(enumerate_ref_stub);
pub var enumerate_ref: *const FN_enumerate_ref = undefined;
...
fn enumerate_new_stub(udev: *Udev) callconv(.c) ?*Enumerator {
    _ = udev;
    return null;
}
...
pub fn load() void {
    var lib = std.DynLib.open("libudev.so.1");

    var loaded = true;
    if (lib) |*l| {
        log.info("Loaded libudev.so", .{});
