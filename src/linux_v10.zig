...
pub fn main() !void {
    ...
    var audio_output: LinuxAudioOutput = .{};
    ...
    audio_output.buffer_byte_size = audio_output.frames_per_second * audio_output.bytes_per_frame / 15;
    audio_output.period_size = 1024;
    audio_output.tone_hz = 256;
    audio_output.latency_frame_count = audio_output.frames_per_second / 15;

    linuxInitAlsa(audio_output.frames_per_second, audio_output.bytes_per_frame, &audio_output.buffer_byte_size, &audio_output.period_size);
    if (requestAudioBufferFill(pcm_opt, &audio_output, audio_output.latency_frame_count)) |f| {
        const pcm = pcm_opt.?;
        @memset(f.frames, .{ 0, 0 });
        f.commit(pcm);
    ...
    while (wl.display_dispatch(display) != -1 and running) {
        ...
        for (&joysticks) |*js| if (js.active) {
            ...
            audio_output.tone_hz = @intFromFloat(512 + 256 * (@as(f32, @floatFromInt(jy)) / 30000));

            // try js.setRumble(3000, 0);
        };

        const audio_fill = requestAudioBufferFill(pcm_opt, &audio_output, audio_write_frame_count);
        ...
        v10.game.updateAndRender(&game_offscreen_buffer, &game_audio_output_buffer, x_offset, y_offset, audio_output.tone_hz);
...
const AudioBufferfill = struct {
    offset: alsa.PcmUFrames,
    frames: [][2]i16,

    fn commit(this: AudioBufferfill, pcm: *alsa.Pcm) void {
        _ = alsa.pcm_mmap_commit(pcm, this.offset, @intCast(this.frames.len));
    }
};

fn requestAudioBufferFill(pcm: ?*alsa.Pcm, audio_output: *LinuxAudioOutput, frames: alsa.PcmSFrames) ?AudioBufferfill {
    if (frames > 0) {
        ...
        assert(area[1] == null);
        var writable_frames: [*][2]i16 = @ptrCast(@alignCast(area[0].?.addr + (offset * audio_output.bytes_per_frame)));
